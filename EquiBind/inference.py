...
def inference(args, tune_args=None):
    sys.stdout = Logger(logpath=os.path.join(os.path.dirname(args.checkpoint), 'inference.log'), syspart=sys.stdout)
    sys.stderr = Logger(logpath=os.path.join(os.path.dirname(args.checkpoint), 'inference.log'), syspart=sys.stderr)
    seed_all(args.seed)
    ...
    for conformer_id in range(args.num_confs):
        ...
        path = os.path.join(os.path.dirname(args.checkpoint),
                             'predictions_Tune{}_RDKit{}_confID{}.pt'.format(tune_args != None, use_rdkit_coords, conformer_id))
        ...
        print('rmsds below 2: {}%'.format((100 * (rmsds < 2).sum() / len(rmsds)).__round__(2)))
        print('rmsds below 5: {}%'.format((100 * (rmsds < 5).sum() / len(rmsds)).__round__(2)))
        ...
        print('centroid_distances below 2: {}%'.format((100 * (centroid_distsH < 2).sum() / len(centroid_distsH)).__round__(2)))
        print('centroid_distances below 5: {}%'.format((100 * (centroid_distsH < 5).sum() / len(centroid_distsH)).__round__(2)))
        ...
        if args.run_corrections:
            ...
            for i, (prediction, target, lig_keypts, rec_keypts, name) in tqdm(enumerate(
                    zip(results['predictions'], results['targets'], results['lig_keypts'], results['rec_keypts'],
                        results['names']))):
                lig = read_molecule(os.path.join('data/PDBBind/', name, '{}_ligand.sdf'.format(name)), sanitize=True)
                if lig == None:  # read mol2 file if sdf file cannot be sanitized
                    lig = read_molecule(os.path.join('data/PDBBind/', name, '{}_ligand.mol2'.format(name)), sanitize=True)
            ...
            print('rmsdvals below 2: {}%'.format((100 * (rmsdvals < 2).sum() / len(rmsdvals)).__round__(2)))
            print('rmsdvals below 5: {}%'.format((100 * (rmsdvals < 5).sum() / len(rmsdvals)).__round__(2)))
            ...
            print('centroid dist below 2: {}%'.format((100 * (centroid_distsU < 2).sum() / len(centroid_distsU)).__round__(2)))
            print('centroid dist below 5: {}%'.format((100 * (centroid_distsU < 5).sum() / len(centroid_distsU)).__round__(2)))
            ...
            print('rmsdvals below 2: {}%'.format((100 * (rmsd_optimized < 2).sum() / len(rmsd_optimized)).__round__(2)))
            print('rmsdvals below 5: {}%'.format((100 * (rmsd_optimized < 5).sum() / len(rmsd_optimized)).__round__(2)))
            ...
            print('centroid dist below 2: {}%'.format((100 * (centroid_dists < 2).sum() / len(centroid_dists)).__round__(2)))
            print('centroid dist below 5: {}%'.format((100 * (centroid_dists < 5).sum() / len(centroid_dists)).__round__(2)))
...
def inference_from_files(args):
    ...
    for idx, name in enumerate(names):
        print('\nProcessing {}: complex {} of {}'.format(name, idx + 1, len(names)))
        ...
        for lig_name in lig_names:
            if not os.path.exists(os.path.join(args.inference_path, name, lig_name)):
                raise ValueError('Path does not exist: {}'.format(os.path.join(args.inference_path, name, lig_name)))
            print('Trying to load {}'.format(os.path.join(args.inference_path, name, lig_name)))
            ...
            if lig != None:  # read mol2 file if sdf file cannot be sanitized
                ...
                break
        if lig_names == []: raise ValueError('No ligand files found. The ligand file has to contain \'ligand\'.')
        if lig == None: raise ValueError('None of the ligand files could be read: {}'.format(lig_names))
        print('Docking the receptor {}\nTo the ligand {}'.format(os.path.join(args.inference_path, name, rec_name), used_lig))
        ...
        with torch.no_grad():
            ...
            if args.run_corrections:
                ...
                if args.output_directory:
                    if not os.path.exists('{}/{}'.format(args.output_directory, name)):
                        os.makedirs('{}/{}'.format(args.output_directory, name))
                    ...
                    print('Writing prediction to {}/{}/lig_equibind_corrected.sdf'.format(args.output_directory, name))
                    with open('{}/{}/lig_equibind_corrected.sdf'.format(args.output_directory, name), "w") as newfile:
                        newfile.write(block_optimized)
            all_names.append(name)

    path = os.path.join(os.path.dirname(args.checkpoint), 'predictions_RDKit{}.pt'.format(use_rdkit_coords))
    print('Saving predictions to {}'.format(path))
...
if __name__ == '__main__':
    ...
    for run_dir in args.run_dirs:
        args.checkpoint = 'runs/{}/best_checkpoint.pt'.format(run_dir)
        config_dict['checkpoint'] = 'runs/{}/best_checkpoint.pt'.format(run_dir)
